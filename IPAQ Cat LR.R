...
################################################################################
# Logistic Regression IPAQ category
################################################################################
...
cross <- cross_all[[i]]
    ...
    cross <- cross %>% mutate(case_wts = ifelse(IPAQ_category == "Yes", 1, 5), 
    ...
    rec_default <- recipe(IPAQ_category ~ ., data = data_train[[i]]) %>%
    ...
    vi_df <- information_gain(IPAQ_category ~ . - case_wts, data = data_train[[i]])
    ...
    data_train[[i]] <- data_train[[i]] %>% dplyr::select(all_of(c(cleaned_vars, "IPAQ_category", "case_wts")))
    data_test[[i]]  <- data_test[[i]] %>% dplyr::select(all_of(c(cleaned_vars, "IPAQ_category")))
    data_test[[i]]  <- data_test[[i]] %>% dplyr::select(all_of(c(cleaned_vars, "IPAQ_category")))

...
for source in time_of_day_order:
    for target in time_of_day_order:
        ...
        selected_columns = ['participant_id', 'day', 'hours_cat', 'timestep', 'date',
            'PHYSICAL_NORM', 'MENTAL_NORM', 'MOTIVATION_NORM', 'EFFICACY_NORM', 'CONTEXT_NORM', 
            'Steps', 'Steps_original', 'Steps_transformed',
            'hours_idx', 'Afternoon', 'Evening',
            'Morning', 'Noon'] + lag_cols
        ...
        diff_features = garmin.filter(regex="^Steps_diff").columns.tolist()
        ...
        sorted_lagged_columns = sorted(X_train.columns, key=lambda x: int(x.split('_')[-1]), reverse=True)

        X_train_seq = X_train[sorted_lagged_columns].values.reshape(X_train.shape[0], X_train.shape[1], 1)
        ...
        X_train_seq = X_train[sorted_lagged_columns].values.reshape(X_train.shape[0], X_train.shape[1], 1)
        X_val_seq = X_val[sorted_lagged_columns].values.reshape(X_val.shape[0], X_val.shape[1], 1)
        X_test_seq = X_test[sorted_lagged_columns].values.reshape(X_test.shape[0], X_test.shape[1], 1)
        ...
        output_path = r"C:\Users\anasn\Desktop\E\Semester 4\Thesis\R code\xgboost timeseries\prediction plots xgboost\results_combination_rnn_lstm_ema.xlsx"

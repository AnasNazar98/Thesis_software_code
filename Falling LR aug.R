...
for (i in 1:10) {
  ...
  cross <- cross %>% mutate(case_wts = ifelse(falling_1 == "Yes", 10, 1), 
...
rec_default <- recipe(falling_1 ~ ., data = data_train[[i]]) %>%
...
vi_df <- information_gain(falling_1 ~ . - case_wts, data = data_train[[i]])
...
data_train[[i]] <- data_train[[i]] %>% dplyr::select(all_of(c(cleaned_vars, "falling_1", "case_wts")))
data_test[[i]]  <- data_test[[i]] %>% dplyr::select(all_of(c(cleaned_vars, "falling_1")))
data_test[[i]]  <- data_test[[i]] %>% dplyr::select(all_of(c(cleaned_vars, "falling_1")))

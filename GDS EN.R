...
table(cross$gds_category)
(start_time <- Sys.time())
for(i in 1:10){
set.seed(seed)
data_folds <- vfold_cv(data_train[[i]], strata = gds_category, v = nrow(data_train[[i]]))
data_folds <- vfold_cv(data_train[[i]], strata = gds_category, v = 10
)
...
rec <- recipe(gds_category ~ ., data = data_train[[i]]) %>%
...
truth <- predictions_list[[1]]$gds_category  
